...
namespace Vtt_Api.CommandAndQueryHandlers.Queries
{
    ...
    public class LookupGuardiansQueryHandler : AbstractVttRequestHandler<LookupQuery<Traveler>, List<Traveler>>
    {
        public readonly VttContext _vttContext;
        public LookupGuardiansQueryHandler(VttContext vttContext)
        {
            _vttContext = vttContext;
        }
        public async override Task<VttHandlerResponse<List<Traveler>>> Handle(LookupQuery<Traveler> request, CancellationToken cancellationToken)
        {
            var query = _vttContext.Travelers.AsQueryable();
